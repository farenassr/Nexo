...
import { useNexoServerModulesRestaurantFeaturesDashboardGetRestaurantDashboardGetRestaurantDashboardEndpoint } from "../../../lib/api/generated/hooks";
...
export function RestaurantDashboardPage() {
  ...
  const dashboardQuery =
    useNexoServerModulesRestaurantFeaturesDashboardGetRestaurantDashboardGetRestaurantDashboardEndpoint(
      { params: { branchId: setup.branchId, date: setup.date } },
      { query: { enabled: canLoadDashboard, staleTime: 30_000 } },
    );
      ...
      <section className="restaurant-module-page">
        ...
        {!canLoadDashboard ? (
          <EmptyState
            ...
            title={labels.states.branchRequired}
          />
        ) : null}
        {dashboardQuery.isPending && canLoadDashboard ? (
          <SkeletonRows count={3} />
        ) : null}
        {dashboardQuery.isError ? (
          <InlineError error={dashboardQuery.error} />
        ) : null}
        {dashboardQuery.data ? (
          <>
            ...
            <div className="dashboard-insights-grid">
              <section className="dashboard-panel">
                <PanelHeader
                  icon={<BarChart3 size={18} />}
                  title="Ocupacion por hora"
                />
                <OccupancyByHourChart
                  points={dashboardQuery.data.occupancyByHour}
                />
              </section>
              <section className="dashboard-panel">
                <PanelHeader
                ...
                <UpcomingReservationsPanel
                  reservations={dashboardQuery.data.upcomingReservations}
                />
              </section>
            </div>
          </>
...
const emptyMetrics: RestaurantDashboardMetric[] = Object.entries(
